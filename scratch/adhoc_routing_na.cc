...
main (int argc, char *argv[])
{
  ...
  std::string CSVfileName = experiment.CommandSetup (argc,argv);
    ...
    LogComponentEnable ("AodvNeighbors", LOG_LEVEL_INFO);
    LogComponentEnable ("AodvRequestQueue", LOG_LEVEL_INFO);
    LogComponentEnable ("AodvRoutingTable", LOG_LEVEL_INFO);
...
//    sinkmobilityAdhoc.Install (adhocNodes);
  ...
  //    ApplicationContainer temp = onoff1.Install (adhocNodes.Get (i));
    ...
    ApplicationContainer apps = server.Install (adhocNodes.Get (0));
    apps.Start (Seconds (1.0));
    apps.Stop (Seconds (10.0));

    Address remote;
    remote = Address (adhocInterfaces.GetAddress (0));
    
    uint32_t MaxPacketSize = 42+12; //UDP adds another 30bytes
    Time interPacketInterval = Seconds (0.000336); // 48 us
    uint32_t maxPacketCount = 10;
    //  uint32_t maxPacketCount = 50;
    UdpClientHelper client (remote, port2);
    client.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
    client.SetAttribute ("Interval", TimeValue (interPacketInterval));
    client.SetAttribute ("PacketSize", UintegerValue (MaxPacketSize));
    apps = client.Install (adhocNodes.Get (1));
    apps.Start (Seconds (1.0));
    apps.Stop (Seconds (3.1));
